...
parser = argparse.ArgumentParser(description='VGS vs. iterations on one player')
parser.add_argument(
...
def approximation(x):
    ...
    fpi = FPI_2P([settings.str_row_init, settings.str_col_init], [settings.payoff_matrix_row, settings.payoff_matrix_col * x])
...
plt.grid(True)
plt.tight_layout()
plt.savefig('vgs_MP_payoff_scale_unilateral.png')
plt.show()
